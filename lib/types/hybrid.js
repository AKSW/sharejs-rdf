...
removeTripleFromTurtleBlock = function(turtleContent, block, blockContentParsed, s, p, o) {
  ...
  return [start, length, turtleContent.substr(start, length), blockStringified];
};
...
RdfJsonTurtleSync = (function() {
  ...
  RdfJsonTurtleSync.prototype._replaceInTextDoc = function(pos, length, oldContent, newContent) {
    if (this.textDoc.substr(pos, length) !== oldContent) {
      throw new Error('Turtle deletion: Text has changed @' + pos + ' (length ' + length + ')');
    }
    return this.textDoc = this.textDoc.substr(0, pos) + newContent + this.textDoc.substr(pos + length);
  };
  ...
  RdfJsonTurtleSync.prototype._applyChangesToTurtle = function(triplesToInsert, triplesToDelete) {
    var length, newContent, oldContent, start, triple, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2, _ref3, _ref4, _results, _results1;
    if (this.textDocParsed) {
      ...
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        triple = _ref1[_j];
        _ref2 = removeTripleFromTurtle(this.textDoc, triple.s, triple.p, triple.o), start = _ref2[0], length = _ref2[1], oldContent = _ref2[2], newContent = _ref2[3];
        _results.push(this._replaceInTextDoc(start, length, oldContent, newContent));
      }
      return _results;
    } else {
