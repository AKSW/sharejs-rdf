...
describe 'hybrid OT emits correct sync events', ->
  ...
  it '(turtle insertion)', ->
    ...
    op = new HybridOp [{ p:193, i:"<http://example.com/persons/andy> <http://example.com/ontology#name> \"Andy Smith\" ." }], {}, {}

    hybridOT.registerDoc doc  # normally done automatically by document/hybrid-api
    hybridOT.apply snapshot, op

    expect(doc.emit).toHaveBeenCalledWith 'sync-rdf', { d: {}, i:
  ...
  it '(turtle deletion)', ->
    ...
    op = new HybridOp [{ p: 193, d: "<http://example.com/persons/andy> <http://example.com/ontology#name>   \"Andy Smith\" ." }], {}, {}

    hybridOT.registerDoc doc  # normally done automatically by document/hybrid-api
    hybridOT.apply snapshot, op

    expect(doc.emit).toHaveBeenCalledWith 'sync-rdf', { i:{}, d:
      'http://example.com/persons/andy':
        'http://example.com/ontology#name':
          [ { type: 'literal', value: 'Andy Smith' } ]
    }
  ...
  it '(rdf/json insertion)', ->
    ...
    expect(doc.emit).toHaveBeenCalledWith 'sync-text-insert', { p:193, i:"\n<http://example.com/persons/andy> <http://example.com/ontology#name> \"Andy Smith\" ." }


  it '(rdf/json deletion)', ->
    turtle = "<http://example.com/persons/john> <http://example.com/ontology#name> \"John Smith\", \"John R. Smith\" ;\n" +
             "                                  <http://example.com/ontology#name> \"John Richard Smith\" .\n" +
             "<http://example.com/persons/andy> <http://example.com/ontology#name>   \"Andy Smith\" ."
    ...
    expect(doc.emit).toHaveBeenCalledWith 'sync-text-replace', { p:193, i:"", d:"<http://example.com/persons/andy> <http://example.com/ontology#name>   \"Andy Smith\" ." }
